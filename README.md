...
![Streamlit kana app deployment architecture](images/streamlit-app-deploy.png)
...
- Docker
- [Domain registered in Amazon Route 53](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/domain-register.html)
...
```
Now, inside the `app.py` code specify your account ID:
```python
# Specify your AWS account ID and region
env = core.Environment(account="YOUR_ACCOUNT_ID", region="us-east-1")
```
Inside the `cdk/cdk_stack.py` code specify your domain name (3 occurrences):
```python
        # HTTPS Certificate
        certificate = acm.Certificate(
            self, "Certificate",
            domain_name="kana.example.com", # specify your domain name
...
```python
        ...
        cloudfront_distribution = cloudfront.Distribution(
            ...
            domain_names=["kana.example.com"], # specify your domain name
        ...
        hosted_zone = route53.HostedZone.from_lookup(self, "HostedZone", 
                                                     domain_name="example.com") # specify your domain name
```
...
$ cdk deploy
```
During deployment, the certificate is created automatically, but you might need to manually validate it. 
Go to the [AWS Certificate Manager](https://console.aws.amazon.com/acm/home), find your certificate with `Pending validation` status and click on it. 
Under `Domains`, choose `Create records in Route 53`. Ensure the records are selected and choose `Create records`. A green banner confirms that you successfully created DNS records. 

When the deployment is complete, open your browser and go to the `kana.example.com` (enter the value specified inside `cdk/cdk_stack.py` according to your domain name), you will see your application.
...
Go to your terminal, clone this repository, checkout to the `deploy-https` branch and cd into `cdk/app`:
```
$ git clone https://github.com/dashapetr/kana--streamlit-app.git
$ cd kana--streamlit-app
$ git checkout deploy-https
...
Now, create virtual environment, activate it, then install all dependencies.
```
$ python -m venv .env
...
### 2 - Deploy your Streamlit app to AWS Fargate using AWS CDK (HTTPS version)
...
Assuming that you have cloned this repo, cd-ed to it, and switched to `deploy-https` branch (See the [Test your application](https://github.com/dashapetr/kana--streamlit-app/tree/deploy-https?tab=readme-ov-file#test-your-application) part), open your terminal and `cd` to cdk.
```
$ cd cdk
```
...
Our CDK app contains a single CDK stack. The CDK app instance is created using the [App](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.App.html) construct. 
The following is the code from the CDK application file `app.py`. 

❗**Don't forget to specify your account ID!**
```python
...
- **Docker Image Deployment**: Builds a Docker image from the local `app` folder and deploys it as a Fargate service.
- **HTTPS Configuration**: Provisions an ACM Certificate for HTTPS (`kana.example.com`) and enables automatic HTTP to HTTPS redirection.
- **Fargate Service with Custom Header Authentication**:  
  Deploys an Application Load Balanced Fargate Service (`StreamlitKanaWebApp`) secured with:  
  - A custom header (`X-Custom-Header`) for access control, generated dynamically using AWS Secrets Manager.  
  - Default denial of requests without the custom header.
- **Auto-Scaling**: Implements auto-scaling for the ECS tasks based on CPU utilization, with configurable cooldown periods.
- **WAF (Web Application Firewall)**: Adds a WAF Web ACL with AWS-managed rules to protect against common web exploits.
- **CloudFront Distribution**:  
  Sets up a CloudFront distribution (`StreamlitKanaCloudFront`) for secure global content delivery with:  
  - HTTPS-only access.  
  - Integration with the custom header for backend access control.
- **Route53 DNS Configuration**:  
  Adds a Route53 A record (`kana.example.com`) to route traffic to the CloudFront distribution.

❗**Important**: specify your domain name (3 occurrences) inside `cdk/cdk_stack.py`

With the [CfnOutput](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.CfnOutput.html) construct, this CDK stack explicitly outputs the following values: 
1. ALB DNS Name: Public address of the Application Load Balancer.  
2. CloudFront Domain Name: Public address of the CloudFront distribution.
...
❗During deployment, the certificate is created automatically, but you might need to manually validate it. Go to the [AWS Certificate Manager](https://console.aws.amazon.com/acm/home), find your certificate with `Pending validation` status and click on it. Under `Domains`, choose `Create records in Route 53`. Ensure the records are selected and choose `Create records`. A green banner confirms that you successfully created DNS records. 

![validate certificate](images/certificate.png)

When deployment completes, the CDK CLI will provide outputs. 

![cdk outputs](images/outputs-https.png)

Now when you open your browser and go to the `kana.example.com` (enter the value you specified inside `cdk/cdk_stack.py` according to your domain name), you will see your application.

Congrats! Your app is online! 🎉

**Note**: If you enter the `KanaStreamlitApp.LoadBalancerAddress` value (your ALB DNS name) in the browser, you should see the `Access denied` message due to the default denial of requests without the custom header.
...
$ cdk destroy
```
![cdk destroy](images/destroy-https.png)
...
- To use a certificate in AWS Certificate Manager (ACM) to require HTTPS between viewers and CloudFront, make sure you request (or import) the certificate in the US East (N. Virginia) Region (us-east-1). (As per official [documentation](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/cnames-and-https-requirements.html))
- CloudFront `Origin request policy` should be set to `AllViewer` or similar because there is a number of Sec-Websocket headers in `streamlit` that need to be forwarded (See the [issue](https://discuss.streamlit.io/t/problems-deploying-streamlit-app-with-aws-cloudfront/8070/6))
- CloudFront `Cache policy` should be `CachingOptimized` (NOT `CachingDisabled`) because `X-Custom-Header` implementation might not be properly handled: `CachingDisabled` does not ensure all viewer headers are forwarded to the origin by default.
...
- Super informative video [Securing a ALB with CloudFront, Enabling HTTPS, SSL Termination & Restricting Direct Access to a ALB](https://youtu.be/pdwGsM2MmDU) on the Hands-On With Digital Den channel.
- AWS official documentation [Restrict access to Application Load Balancers](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/restrict-access-to-load-balancer.html)
...
- Regular rotation of secrets (`X-Custom-Header` values) is recommended, can be configured additionally.